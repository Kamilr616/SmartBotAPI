...
- Arduino IDE 2.x with the ESP32 board package, **or** PlatformIO
...
1. **Create the secrets file** `src/arduino/sketch_robot_signalr/arduino_secrets.h` (git-ignored):
   ...
   All three pairs must be defined (they may repeat). The firmware connects to whichever network is available.

2. **Point the firmware at your server** in `config.h`:

   ```cpp
   #define SERVER_IP   "smartbotweb.azurewebsites.net"  // or your LAN IP / hostname
...
3. **Upload** — Arduino IDE: select board *ESP32C3 Dev Module / DevKitM-1*, open `sketch_robot_signalr.ino`, click Upload. PlatformIO alternative:

   ```bash
   cd other/241111-183051-esp32-c3-devkitm-1
...
1. Open the dashboard and navigate to **Chat** (`/chat`).
...
|---|---|
| Robot never connects | Wrong `SERVER_IP`/`SERVER_PORT` in `config.h`; TLS port used against a plain-HTTP server; firewall blocking the port |
