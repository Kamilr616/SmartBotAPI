...
namespace SmartBotWebAPI
{
    public class SignalHub : Hub
    {
        ...
        public async Task ReceiveRobotData(string user, double temperature, double[] measurements, ushort[] rawMatrix, ushort avgDistance)
        {
            ushort[] interpolatedMatrix = InterpolateData(rawMatrix, 32);

            await Clients.Others.SendAsync("ReceiveMatrix", $"API SignalHub -> {user}", temperature,  measurements, interpolatedMatrix, avgDistance);
        }
        ...
        private static ushort[] InterpolateData(ushort[] data, int targetSize = 32)
        {
            if (data.Length != 64)
            {
